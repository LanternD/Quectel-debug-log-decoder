...
class GpsController(QThread):
    ...
    def __init__(self, com, rate):
        ...
        self.ignored_msg_list = [b'$GBGSV', b'$GPGSV', b'$GNZDA', b'$GNGSA']  # ignore some messages to save time.
    ...
    def run(self):
        ...
        while self.streaming_flag:
            buf = self.read_gps(18)  # the number here depends on the device.
    ...
    def read_gps(self, amount):
        ...
        for i in range(amount):
            byte_buffer = self.ser.readline()[:-2]  # one message per line. remove the '\r\n'
            if byte_buffer[:6] in self.ignored_msg_list:
                continue
            info_buffer.append(byte_buffer.decode('utf-8'))
        # print(info_buffer)
        return info_buffer
